...
async def test_subscription_purchase_creates_fixed_price_payment(db_session):
    ...
    subscription = await subscription_service.purchase(client.id, SubscriptionType.MONTHLY)
...
async def test_payment_history_shows_subscription_purchases_only(db_session):
    ...
    await subscription_service.purchase(client.id, SubscriptionType.MONTHLY)
    db_session.add(
        Payment(
            user_id=client.id,
...
async def test_subscription_purchase_rejects_second_active_membership(db_session):
    ...
    with pytest.raises(HTTPException) as error:
        await subscription_service.purchase(client.id, SubscriptionType.YEARLY)
