...
| Класи | ні | ні | так | так | так |
| Користувачі | ні | ні | ні | так | так |
| Звіти | ні | ні | ні | так | так |
| Публічні плани | так | так | так | так | так |

Додатково для модуля занять діє окреме правило авторизації:

- підтвердження завершення заняття дозволене тренеру власного класу;
- `ADMIN` і `OWNER` можуть підтверджувати завершення будь-якого класу;
- до фактичного `end_time` endpoint підтвердження повертає помилку доступності сценарію.
