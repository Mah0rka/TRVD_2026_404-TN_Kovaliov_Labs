...
// Перетворює сирі dashboard-дані на готові до рендера блоки з role-specific копірайтом і метриками.
export function buildDashboardViewModel({
  ...
  const trainerAttendance = myClasses.reduce(
    (total, item) =>
      total + item.bookings.filter((booking) => booking.status === "CONFIRMED").length,
    0
  );
  ...
  // Hero/focus/stat секції залишаються чистими даними, щоб page-компонент був лише thin renderer.
  ...
  const focusMeta = isClient
    ...
    : isTrainer
      ...
      : [
          ...
          `₴${revenue?.total_revenue.toLocaleString("uk-UA") ?? "0"} за 30 днів`
        ];
    ...
    : isTrainer
      ...
      : [
          ...
          },
          {
            ...
            value: `₴${revenue?.total_revenue.toLocaleString("uk-UA") ?? "0"}`,
            note: "видно одразу з кабінету"
          },
          {
            label: "Транзакцій",
            value: revenue?.transactions_count ?? 0,
    ...
    : isTrainer
      ...
      : [
          ...
          },
          {
            ...
            to: "/dashboard/payments",
            badge: `${revenue?.transactions_count ?? 0} транзакцій`,
            tone: "light"
          }
        ];
